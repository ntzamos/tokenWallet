...
function format(value, decimals, precision) {
    while(value.length <= decimals) value = "0" + value;
    var ans = value.slice(0,value.length-decimals) + "."+ value.slice(value.length-decimals,value.length-decimals+precision);
...
function preloader(id, color) {
    ...
    $(id).html(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="400" height="100">
      ...
      <circle transform="translate(16 0)" cx="0" cy="16" r="0" stroke="${color}" fill="transparent">
        <animate attributeName="r" values="0; 4; 0; 0" dur="1.2s" repeatCount="indefinite" begin="0.3"
          keytimes="0;0.2;0.7;1" keySplines="0.2 0.2 0.4 0.8;0.2 0.6 0.4 0.8;0.2 0.6 0.4 0.8" calcMode="spline" />
      </circle>
      <circle transform="translate(24 0)" cx="0" cy="16" r="0" fill="transparent" stroke="${color}">
...
function renderBalance(contractAddress, address) {
  ...
  $.getJSON(`https://api.etherscan.io/api?module=account&action=tokenbalance&contractaddress=${contractAddress}&address=${address}&tag=latest&apikey=${etherscanApiKey}`, function(data){
      $("#balance").html(`<span id="myBalance">${format(data.result, 18, 4)}</span> <span id="tokenSymbol" style="display:none;">${symbol}</span>`);
      console.log(format(data.result, 18, 4))
  });
}

function renderTransactions(contractAddress, address) {
  preloader("#myTransactions", '#12b8ff');
  $.getJSON(`https://api.etherscan.io/api?module=logs&action=getLogs&fromBlock=4042013&toBlock=latest&address=${contractAddress}&apikey=${etherscanApiKey}&topic0=0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef&topic1_2_opr=or&topic1=0x000000000000000000000000${address.substr(2)}&topic2=0x000000000000000000000000${address.substr(2)}`, function(data){
    ...
    transactions.forEach(function(t) {
      ...
      var total = parseFloat(format(gasPrice.mul(gasUsed).toString(10),18, 4));
      ...
      var month = new moment(parseInt(t.timeStamp,16)*1000).format("MMYY");

      if( !(month in months) ) {
        
        months[month] = 1;

        transactionsView += `<div class="transaction">
                                <div class="row">
                                  <div class="col-auto">
                                    <center><small>${new moment(parseInt(t.timeStamp,16)*1000).format("MMMM YYYY")}</small></center>
                                  </div>
                                </div></div>`;
      ...
      transactionsView += `<div class="transaction">
                            <div class="row">
                              <div class="col-auto">
                                ${new moment(parseInt(t.timeStamp,16)*1000).format("DD MMM HH:mm:ss")}
                              </div>
                              <div class="col-auto" style="text-align: right">
                                <span class='label label-${other==from?'success':'warning'}'>${other==from?'':'-'}${format(a,18, 4)} </span>
                              </div>
                            </div>
                            ...
                            <div class="row show-more" style="display:none;">
                              <div class="col-auto">
                                <small><a href='${other}'>${other.substr(0,other.length/2)}...</a></small>
                              </div>
                              <div class="col-auto" style="text-align: right">
...
$("#myTransactions").on('click', '.transaction', function(){
  console.log($(this).find('.show-more').toggle())
});
...
$('.page-content').on('scroll', function (){ 
  const distanceY = $('.page-content').scrollTop(),
  shrinkOn = 100;
  
  if (distanceY > shrinkOn) {
    $(".title").html($("#myBalance").html() + " CRTT");
    $("#balance").hide();
    $(".navbar-inner").css("border-bottom", "1px solid rgba(0, 0, 0, 0.12)");
    $(".navbar-inner").css("background", "white");
  } else {
    $(".title").html("CRTT");
    $("#balance").show();
    $(".navbar-inner").css("border-bottom", "0px");
